...
class Country(DBO):
    def insert(self, name, alpha2Code, alpha3Code, population, region_id):
        insert_query = (
            "INSERT INTO country (name, alpha2Code, alpha3Code, population, region_id, topLevelDomain, capital) VALUES (?, ?, ?, ?, ?, ?, ?)"
        )
        self.cursor.execute(
    ...
    def update(self, name, topLevelDomain, capital):
        update_query =( 
            "UPDATE country SET topLevelDomain=?, capital=? WHERE name=?" 
        )
        self.cursor.execute(
            update_query, (topLevelDomain, capital, name)
        )
        conn.commit()
        self.get_by_name(name)
